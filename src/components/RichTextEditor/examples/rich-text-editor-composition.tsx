			...
			{isEditing ? (
				...
				<VStack gap={2} align='stretch'>
					<HStack justify='space-between'>
						<Text fontSize='sm' lineClamp={1} truncate flex='1' color='blue.600'>
							{url}
						</Text>
					</HStack>
					<HStack gap={2}>
						<Button size='xs' variant='outline' onClick={() => setIsEditing(true)} flex='1'>
							Edit
						</Button>
						<Button size='xs' variant='outline' colorPalette='red' onClick={handleRemove} flex='1'>
							Remove
						</Button>
					</HStack>
				</VStack>
	...
	return (
		<Flex px={4} py={2} alignItems='center' justifyContent='space-between'>
						...
						{menuItems.map((menu) => (
							<MenuRoot key={menu.label}>
								...
								<MenuContent minW='200px' py={1}>
									{menu.items.map((item) => (
										<MenuItem value={item.label} key={item.label} gap={3} cursor='button'>
			...
			<HStack gap={4}>
				...
				<Button borderRadius='full' px={6} gap={2}>
					<LuLock size={14} />
					Share
				</Button>
...
const Toolbar = () => {
	return (
		<Box px={4}>
			<HStack bg='bg.muted' p={2} gap={1} rounded='50px' mt='4' overflowX='auto'>
				...
				{headings.length === 0 ? (
					...
					headings.map((h) => (
						<Button
							...
							truncate
						>
							<Icon as={LuFileText} mr={2} flexShrink={0} color='gray.500' boxSize={4} />
							{h.text}
						</Button>
					))
				)}
								...
								<Tabs.Root defaultValue='url'>
									...
									<Tabs.Content value='url'>
										<Box display='flex' gap='2' mt='4'>
											<Input placeholder='Enter image URL' id='image-url-input' />
											<Button
												onClick={() => {
													const url = (
