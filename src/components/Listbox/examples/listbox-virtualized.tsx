...
import { createListCollection, Listbox, useLiveRef } from '@chakra-ui/react'
...
export const ListboxVirtualized = () => {
  ...
  const collection = useMemo(() => createListCollection({ items: countries }), [])

  return (
    <Listbox.Root collection={collection} maxW="sm" scrollToIndexFn={virtual.scrollToIndexFn}>
      <Listbox.Label>Select Country ({countries.length} items)</Listbox.Label>
      <Listbox.Content maxH="300px" ref={virtual.scrollRef}>
...
function useListboxVirtualizer(props: { count: number }) {
  ...
  return {
    ...
    getViewportProps(props: React.ComponentProps<'div'> = {}): React.ComponentProps<'div'> {
      return {
        ...props,
        style: {
          ...props.style,
