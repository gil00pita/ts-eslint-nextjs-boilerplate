    ...
    <Box>
      <RichTextEditor.Root borderWidth="1px" editor={editor} rounded="l2">
        <RichTextEditor.Toolbar>
          ...
          <HStack flex="1" gap="2" justify="flex-end">
            <Badge
              colorPalette={
                saveStatus === 'saving' ? 'yellow' : saveStatus === 'saved' ? 'green' : 'gray'
              }
              variant="subtle"
            >
              <HStack gap="1">
                {saveStatus === 'saving' && <LuLoader className="animate-spin" />}
                ...
                <Text>
                  ...
                  {saveStatus === 'idle' &&
                    (lastSaved ? `Saved ${formatLastSaved(lastSaved)}` : 'Draft')}
                </Text>
              </HStack>
            </Badge>
          </HStack>
